...
import { User, Session as AuthSession, AuthError } from '@supabase/supabase-js'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => { if (!initialized.current) setLoading(false) }, 3000)
  ...
  const signOut = async () => { setProfile(null); setUser(null); await supabase.auth.signOut() }
  const resetPassword = async (email: string) => { const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: `${window.location.origin}/reset-password` }); return { error } }
  ...
  const refreshProfile = async () => { if (user) { const prof = await loadProfile(user.id); if (prof) setProfile(prof) } }

  const role = profile?.role
  return (
    <AuthContext.Provider value={{
      user, profile, session, loading,
      isAdmin: requireAdmin(role), isStaff: requireStaff(role),
      canAccessFinancial: canAccessFinancialData(role), canManageSession: canManageSessions(role), canViewAudit: canViewAuditLogs(role),

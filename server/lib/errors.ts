 ...
 *
 * PROBLEM: Raw database/API errors expose internal details
...
export interface AppError {
  ...
  internalError?: unknown; // For logging only
}
  ...
  if (error instanceof Error) {
    ...
    for (const [code, arabicMsg] of Object.entries(DB_ERROR_CODES)) {
      if (msg.includes(code)) {
        return { message: arabicMsg, code, isRetryable: false, internalError: error };
      }
    }

    // Check for MySQL error codes (format: SQLSTATE or error number)

...
-- ─────────────────────────────────────────────────────────────
-- 6. PUBLIC SESSION STATUS (for QR code page — no login needed)
-- Allows anon users to read active session info for a device
-- Only exposes: id, started_at, mode, game_played, device info
-- ─────────────────────────────────────────────────────────────

-- Allow anon to read active sessions (limited fields via view)
CREATE OR REPLACE VIEW public_session_status AS
SELECT
  s.id,
  s.started_at,
  s.mode,
...
-- Grant anon read on this view only
ALTER VIEW public_session_status OWNER TO postgres;
GRANT SELECT ON public_session_status TO anon;

-- Also allow anon to read devices table (device name for idle state)
CREATE POLICY "anon_read_devices" ON devices FOR SELECT TO anon
  USING (TRUE);

-- Also allow anon to read active sessions (only ended_at IS NULL)
CREATE POLICY "anon_read_active_sessions" ON sessions FOR SELECT TO anon
  USING (ended_at IS NULL);

-- Allow anon to read customers (name only — for QR page)
CREATE POLICY "anon_read_customers_name" ON customers FOR SELECT TO anon
  USING (TRUE);
